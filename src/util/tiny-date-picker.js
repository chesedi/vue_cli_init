...
(function(global, factory) {
  ...
  function DatePickerOptions(opts) {
    ...
    const parse = dateOrParse(opts.parse);
  ...
  const CustomEvent = shimCustomEvent();
  ...
  function render(dp) {
    ...
    return (
      ...
      mapDays(hilightedDate, dayOffset, function(date) {
        ...
        const isSun = !date.getDay();
        let className = 'dp-day';
        className += isNotInMonth ? ' dp-edge-day' : isSun ? ' dp-sunday':'';
  ...
  function adjustCalX(dp, inputPos, win) {
    ...
    const left =
      calRight > maxRight && shiftedLeft > 0 ? shiftedLeft : inputLeft;
